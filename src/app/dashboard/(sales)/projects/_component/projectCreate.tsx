...
const ProjectCreate: React.FC<ProjecctCreateProps> = ({ close }) => {
  ...
  const items = [
    {
      title: "1. 12A PROVISIONAL REGISTRATION",
      description: "dsjlvgbdfsivu sd/li yvlsdi",
    },
  ...
  const addOns = fixedwise.filter((item) => item.fixedType === "Module_Fees");
  const [moduleFees, setModuleFees] = useState<FixedGovtWise[]>([]);
  const govtFees = fixedwise.filter(
    (item) => item.fixedType === "Government_Fees"
  );
  const profFees = fixedwise.filter(
    (item) => item.fixedType === "Professional_Fees"
  );
  const toggleRing = (item: FixedGovtWise, index: number) => {
    // Toggle active indices
  ...
  const [promoterWiseFees, setPromoterWiseFees] =
    useState<PromoterWiseFee | null>(null);

  useEffect(() => {
    const fetchPromoterWiseFees = async () => {
      const session = await getSession();
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/promoters/promoter-wise-fee/${selectedServiceId}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${session?.user.accessToken}`,
  ...
  useEffect(() => {
    const fetchStateWiseFees = async () => {
      const session = await getSession();
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/promoters/state-wise-fee/${selectedServiceId}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${session?.user.accessToken}`,
  ...
  const [selectedStateCapital, setSelectedStateCapital] =
    useState<string>("100000");
  const handleStateFeeSelectChange = (
    event: React.ChangeEvent<HTMLSelectElement>
  ) => {
    setSelectedStateCapital(event.target.value);
  };
  const [stateFee, setStateFee] = useState<number>(0);
  useEffect(() => {
    console.log(selectedStateCapital);
    if (selectedStateCapital && stateWiseFees) {
      console.log(selectedStateCapital);
      console.log(stateWiseFees.selectedStateCapital);
      console.log(stateWiseFees[selectedStateCapital]);
      setStateFee(
        stateWiseFees[selectedStateCapital].reduce((acc, fee) => {
          console.log(acc);
          return acc + fee.amount;
        }, 0)
      );
    }
  }, [selectedStateCapital, stateWiseFees]);
  const [governmentFee, setGovernmentFee] = useState<number>(0);
  const [modulesFee, setModulesFee] = useState<number>(0.0);
  const [professionalFees, setProfessionalFees] = useState<number>(0.0);
  const gstRate = 0.18;
  useEffect(() => {
    setGovernmentFee(
      ...
      <div>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder={
                        businessStorage?.businessName || "Business Name"
                      }
                      {...field}
                    />
                  </FormControl>
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent>
                        {
                          businessStorage?.businessUsers.map((user,index)=>{
            ...
            <Button
              type="submit"
              className="mt-4 w-full bg-[#f32100] text-white hover:bg-[#f32100]/70"
            >
              Save & Continue
            </Button>
          </form>
          ...
          {stateWiseFees && (
            <div>
              ...
              <select
                ...
                value={selectedStateCapital} // Bind the selected value to state
                onChange={handleStateFeeSelectChange}
              >
                <option value="100000">upto 1 Lac</option>
                <option value="500000">upto 5 Lac</option>
                <option value="1000000">upto 10 Lac</option>
                <option value="1500000">upto 15 Lac</option>
              </select>
            </div>
          )}
          {promoterWiseFees && (
            <div className="grid grid-cols-2 gap-1">
              <div>
                ...
                >
                  ...
                  <option value="Four_Promoters">4 Promoters</option>
                  <option value="Five_Promoters">5 Promoters</option>
                  <option value="Seven_Promoters">7 Promoters</option>
                  <option value="Eight_Promoters">8 Promoters</option>
                  <option value="Ten_Promoters">10 Promoters</option>
                </select>
              </div>
      ...
      <div className="max-w-fit mx-auto bg-white rounded-lg text-xs">
        ...
        <div className="bg-red-50 p-3 rounded-xl flex items-center justify-between shadow-sm">
          <div className="flex items-center space-x-2">
            ...
            <div>
              <div className="text-[#f32100] font-semibold text-sm">
                Wallet Balance
              </div>
          ...
          <div className="flex items-center space-x-2">
            <button className="bg-[#f32100] text-white p-1 rounded-full text-sm shadow-md">
              <svg
                ...
                viewBox="0 0 24 24"
              >
                <path d="M12 2a10 10 0 1010 10A10 10 0 0012 2zm1 14.93V16h-2v.93A8.12 8.12 0 014.07 13H6v-2H4.07A8.12 8.12 0 0111 4.07V6h2V4.07A8.12 8.12 0 0119.93 11H18v2h1.93A8.12 8.12 0 0113 16.93zM12 14a2 2 0 112-2 2 2 0 01-2 2z" />
              </svg>
            </button>
            <button className="bg-[#f32100] text-white p-1 rounded-full text-sm shadow-md">
              <svg
                ...
                viewBox="0 0 24 24"
              >
                <path d="M11 9h2v6h-2zm1 13a10 10 0 100-20 10 10 0 000 20zm0-18a8 8 0 11-8 8 8 8 0 018-8z" />
              </svg>
            </button>
          </div>
        </div>
  ...
  const fourthStep = () => {
    return (
      <div className="max-w-sm mx-auto bg-white p-4 text-center space-y-1">
        ...
        <div>
          <button
            className="w-full bg-[#f32100] text-white py-2 rounded-md font-medium text-xs hover:bg-[#f32100]"
