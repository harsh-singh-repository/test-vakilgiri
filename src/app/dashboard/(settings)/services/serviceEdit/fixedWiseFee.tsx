...
export interface FixedGovtWise {
  id: string;
  amount: number;
  title: string;
  description: string;
...
export const fetchFixedWise = async (serviceId: string) => {
  const session = await getSession();
  try {
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/promoters/fixed-wise-fee/${serviceId}`,
      {
        headers: {
...
const FixedWiseFee: React.FC<FixedWiseProps> = ({ data }) => {
  ...
  const fetchData = async () => {
    if (data.id) {
      const fetchedData = await fetchFixedWise(data.id);
  ...
  return (
    <div>
      <div className="flex gap-2 w-full">
        <p className="text-[#091747] font-bold font-poppins text-[17px]">
          Fixed Govt Fees
        </p>
        <div
                  ...
                  <tr key={item.id}>
                    <td className="px-4 py-2">
                      ...
                      <div className="flex gap-2 font-poppins text-[12px]">
                        <strong className="font-poppins text-[12px]">{`Type : `}</strong>
                        {item.fixedType}
                      </div>
                    </td>
