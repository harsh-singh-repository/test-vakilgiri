  ...
  return (
    <div className="p-5 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
      {cardData.map((card, index) => {
        return (
          <div key={index} className={`w-full xl:order-1 flex justify-start items-center border border-gray-300 rounded-xl shadow-md shadow-gray-300 ${card.BgColor}`}>
              ...
              <div className="w-full flex justify-center gap-x-2 items-center p-2">
                ...
                <div className="w-full flex justify-between items-center gap-1">
                  {card.data.map((data, dataIndex) => {
                    return (
                      <div className="flex flex-col justify-center items-center gap-x-2 text-xs" key={data.label + dataIndex}>
                        <span className={`text-lg font-semibold ${card.textColor}`}>{data.value}</span>
                        <span className={`text-sm font-medium ${card.textColor}`}>{data.label}</span>
                      </div>
                    );
                  })}
                </div>
